const lista1 = ["artless", "bawdy", "beslubbering", "bootless", "churlish", "cockered", 
...
const lista2 = ["base-court", "bat-fowling", "beef-witted", "beetle-headed", "boil-brained",
...
"scurvy-valiant", "brazen-faced", "unwash'd", "bunch-back'd", 
"leaden-footed", "muddy-mettled", "pigeon-liver'd", "scale-sided"];

const lista3 = ["apple-john", "baggage", "barnacle", "bladder", "boar-pig",
...
"knave", "blind-worm", "popinjay", "scullion", "jolt-head",
"malcontent", "devil-monk", "toad", "rascal", "Basket-Cockle"];

function generarFrase() {
    const palabra1 = lista1[Math.floor(Math.random() * lista1.length)];
    const palabra2 = lista2[Math.floor(Math.random() * lista2.length)];
    const palabra3 = lista3[Math.floor(Math.random() * lista3.length)];
    const frase = `Thou, ${palabra1} ${palabra2} ${palabra3}`;
    document.getElementById("fraseGenerada").textContent = frase;
}

function esborrarInsult() {
    const fraseGeneradaElement = document.getElementById("fraseGenerada");
    fraseGeneradaElement.textContent = "";
}
